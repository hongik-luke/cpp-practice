...
// 간선이 큰 것부터 연결시킨다 
// 1) 만약 같은 집합이라면 무시한다.
// 2) 이 때 각 집합의 크기를 곱한 수 에 (전체에서 가중치만큼을 계속 뺴줬던 값)을 곱해서 정답에 더하고 나눠준다.
...
//유니온파인드 그리고 사이즈로 집합의 갯수 체크
struct dis{
    vector<int> parent,rank;
    vector<int64_t> size; // 수정!
...
void get_result(){
    ...
    while(!pq.empty()){
        ...
        result += all*(a.getsize(n.from)*a.getsize(n.to)); // 역시 여기 연산에서 인트형을 초과하면서 문제가 생겼었군
        result %= 1000000000;
        a.marge(n.from, n.to);
        all -= n.value;
    }
